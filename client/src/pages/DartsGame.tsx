...
import type { Match, TournamentConfig, TournamentState, StandingsEntry } from "@/types/tournament";
...
export default function DartsGame() {
  ...
  const tournamentMatchCompleteCallbackRef = useRef<((winnerId: string, legsA: number, legsB: number, setsA: number, setsB: number) => void) | null>(null);
  ...
  const resetLeg = () => {
    setGamePlayers((prevPlayers) => prevPlayers.map((p) => ({
      ...p,
  ...
  const handleTournamentMatchStart = (match: Match, config: TournamentConfig, onMatchComplete: (winnerId: string, legsA: number, legsB: number, setsA: number, setsB: number) => void) => {
    ...
    tournamentMatchCompleteCallbackRef.current = onMatchComplete;
  ...
  const handleBackToTournament = () => {
    ...
    tournamentMatchCompleteCallbackRef.current = null;
    setGamePlayers([]);
    setCurrentPlayerIndex(0);
    setCurrentDartIndex(0);
    setThrowHistory([]);
  ...
  const handleTournamentMatchComplete = (matchId: string, winnerId: string, legsPlayerA: number, legsPlayerB: number, setsPlayerA: number, setsPlayerB: number) => {
    setTournamentState(prev => {
      ...
      if (playerA) {
        ...
        playerA.wins += winnerId === match.playerAId ? 1 : 0;
        playerA.losses += winnerId === match.playerAId ? 0 : 1;
      }
      ...
      if (playerB) {
        ...
        playerB.wins += winnerId === match.playerBId ? 1 : 0;
        playerB.losses += winnerId === match.playerBId ? 0 : 1;
      }
      ...
      return {
        ...
        activeMatchId: null,
        phase: allMatchesCompleted ? ("completed" as const) : ("active" as const),
      } as TournamentState;
    });
  };
    ...
    if (newScore === 0) {
      ...
      if (newLegs >= numberOfLegs) {
        if (newSets >= numberOfSets) {
          ...
          if (isTournamentMode && tournamentMatch && tournamentMatchCompleteCallbackRef.current) {
            ...
            if (playerA && playerB) {
              ...
              setTimeout(() => {
                if (tournamentMatchCompleteCallbackRef.current) {
                  console.log('Calling tournament callback with:', {
                    winnerId: finalPlayers[currentPlayerIndex].id,
                    legsA: playerA.totalLegsWon || 0,
                    legsB: playerB.totalLegsWon || 0,
                    setsA: playerA.sets,
                    setsB: playerB.sets,
                    playerAId: playerA.id,
                    playerBId: playerB.id
                  });
                  tournamentMatchCompleteCallbackRef.current(
